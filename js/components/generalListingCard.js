import { rating } from "./rating.js";

export function generalListingCard(item) {
    let tag = '';
    if (item.operatingHours.alwaysOpen) {
        tag = '<div class="tag tag-24">24 hours</div>';
    } else if (false) {
        tag = '<div class="tag tag-open">Open</div>';
    } else {
        tag = '<div class="tag tag-closed">Closed</div>';
    }
    const ad = item.isAd ? '<span class="ad">Ad</span>' : '';
    const verified = item.isVerified ? '<div class="verified">✔</div>' : '';
    const location = `<div class="detail">ICON + ${item.details.location}</div>`;
    const phone = item.details.phone ? `<div class="detail">ICON + ${item.details.phone}</div>` : '';

    return `
        <div class="general-listing-card">
            <div class="visual">
                <img src="./img/${item.imgSrc}" alt="${item.imgAlt}">
                ${tag + rating(item.rating)}
            </div>
            <div class="name">${ad + item.name + verified}</div>
            <div class="details">${location + phone}</div>
            <div class="footer">
                <a class="search" href="${item.searchHref}">ICON</a>
                <button class="favorite">♥</button>
                <a class="more" href="${item.innerPageHref}">Details + ICON</a>
            </div>
        </div>`;
}